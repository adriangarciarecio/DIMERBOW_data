...
def get_angle_pymol(coord_a, coord_b, v_ref):
    ...
    cos_angle = np.dot(v_ref, vector_b)/ (np.linalg.norm(v_ref) * np.linalg.norm(vector_b))
    #print (cos_angle)
...
if __name__ == "__main__":
    ...
    alldistinfo = open(f'{path_results}dimer_dist_5.txt','w')
    allangleinfo = open(f'{path_results}dimer_angles_5.txt', "w")
